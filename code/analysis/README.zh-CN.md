...
`data/validation/label_assessment/assessment_input_bounds.csv`（边界标识为
`released_integer_frames`）；评估配置为`canonical2556`，不能直接用不同边界新提特征替换评估数组。
...
标签评估使用与配套提取区间匹配的数值输入及现行标签，并包含127对结构内family比较。
`retry-seed`只处理训练折类别覆盖的可行性，完整记录尝试种子和训练／测试成员，
不按预测成绩选择分折。

聚类从`assets/clustering/candidate_inputs/`读取2,893行特征及配套RMS、来源身份。
这套候选特征与RMS必须成套使用，不能与标签评估特征库互换；
聚类结果用于人工比较，不直接代表最终family标签。
